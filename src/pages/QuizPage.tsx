...
import { getQuiz, type Quiz } from "../data/quiz";
import { usePersistentState } from "../hooks/usePersistentState";
import { quizScoreKey, type QuizScore } from "../data/storageKeys";

// A single knowledge check on its own page (linked from the Knowledge Checks
// index). Only this quiz's questions load here.
export default function QuizPage() {
  ...
  if (!quiz) {
    return (
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-6 rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-card dark:border-slate-800 dark:bg-slate-800/50">
          <p className="text-slate-600 dark:text-slate-300">
            That knowledge check could not be found.
          </p>
        </div>
      </div>
    );
  ...
  const guide = courses.find((c) => c.quizId === quiz.id);
      ...
      <header className="mt-5">
        <div className="flex items-center gap-2.5">
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
            {guide ? guide.title : "Knowledge check"}
          </p>
        </div>
